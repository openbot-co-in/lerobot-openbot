...
from lerobot.common.robots.giraffe_follower import (
    GiraffeFollower,
    GiraffeFollowerConfig,
)
...
def follower():
    ...
    with (
        patch(
            "lerobot.common.robots.giraffe_follower.giraffe_follower.FeetechMotorsBus",
            side_effect=_bus_side_effect,
        ),
        patch.object(GiraffeFollower, "configure", lambda self: None),
    ):
        cfg = GiraffeFollowerConfig(port="/dev/null")
        robot = GiraffeFollower(cfg)
        yield robot
